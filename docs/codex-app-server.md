...
6. 在第 4 段选择是否“允许 AI 读取当前终端内容”。关闭时 Codex 完全不会获得当前 SSH/本地终端文字。
...
- 开启终端上下文权限后，只注册 `terminal_read`；不注册 `terminal_execute` 或 `terminal_state`。
...
- 可选 `terminal_read` 测试覆盖开/关权限，并断言 Codex 路径不获取终端控制租约。
